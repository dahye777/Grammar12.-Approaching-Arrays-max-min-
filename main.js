...
console.log(max);
console.log(min);
/*apply()는 매개변수로 함수에서 사용할 this객체와 호출하는 함수를 입력받습니다
this객체는 나중에 문법으로 배울 예정입니다
위의 코드에서 함수 배부에서 사용할 this객체가 없기 때문에 null을 사용한 것

es5에서는 apply() 함수호출 메서드를 사용해서 각 배열의 값을 접근했습니다
즉 불변성 유지를 달성한 상태에서 배열을 복사하는 방법이 없었기 때문에
...
console.log(maxV);
console.log(minV);
// es6의 전개연산자를 사용하면 전개연산자를 사용해서 불변성을 유지하면서 인수값으로 arr을 직접 넣어주면 끝나는
// 쉽고 가독성도 좋은 방법입니다
...
const rich = guys.filter((el, index) => {
...
복사하고 Math.max 이곳에 인수로 담도록 하는 코드입니다
*/
